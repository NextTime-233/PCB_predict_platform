<template>
    <div>
        <el-input v-model="addressKeyword" placeholder="请输入地址来直接查找相关位置"></el-input>
        <!-- 地图界面 -->
        <baidu-map class="map" :scroll-wheel-zoom="true" center="沈阳" :zoom="zoom" @click="getLocationPoint">
        <!-- 比例尺-->
            <bm-scale anchor="BMAP_ANCHOR_TOP_RIGHT"></bm-scale>
            <!-- 比例尺-->
        <!-- <bm-point-collection :points="points" shape="BMAP_POINT_SHAPE_STAR" color="blue" size="BMAP_POINT_SIZE_SMALL" @click="clickHandler"></bm-point-collection>-->
        <!-- <bm-view style="width: 100%;  height: 700px;  margin-top: 10px;"></bm-view>-->
            <!-- 热力图 -->
            <bml-heatmap :data="data" :max="100" :radius="20">
            </bml-heatmap>
            <!-- 聚类中心标识 -->
            <bm-view style="width: 100%; height:600px; flex: 1"></bm-view>
            <bm-local-search :keyword="addressKeyword" :auto-viewport="true" style="display: none"></bm-local-search>

            <bm-marker  :position="{lng: 123.419112, lat: 41.808}" :dragging="true" animation="BMAP_ANIMATION_BOUNCE" @click="lookDetail(markers[0])">
                <bm-label content="A"
                          :labelStyle="{color: '#fff', fontSize : '12px',background:'rgba(0, 0, 0, 0)',borderColor:'rgba(0, 0, 0, 0)'}"
                          :offset="{width: 4,height:2 }"
                />
            </bm-marker>
            <bm-marker :position="{lng: 122.914, lat: 42.013}" :dragging="true" animation="BMAP_ANIMATION_BOUNCE" @click="lookDetail(markers[1])">
                <bm-label content="B"
                          :labelStyle="{color: '#fff', fontSize : '12px',background:'rgba(0, 0, 0, 0)',borderColor:'rgba(0, 0, 0, 0)'}"
                          :show="false"
                          :offset="{width: 4,height:2 }"
                />
            </bm-marker>
            <bm-marker :position="{lng: 123.523, lat: 42.430}" :dragging="true" animation="BMAP_ANIMATION_BOUNCE" @click="lookDetail(markers[2])">
                <bm-label content="C"
                          :labelStyle="{color: '#fff', fontSize : '12px',background:'rgba(0, 0, 0, 0)',borderColor:'rgba(0, 0, 0, 0)'}"
                          :offset="{width: 4,height:2 }"
                />
            </bm-marker>
            <bm-marker :position="{lng: 122.506, lat: 41.458}" :dragging="true" animation="BMAP_ANIMATION_BOUNCE" @click="lookDetail(markers[3])">
                <bm-label content="D"
                          :labelStyle="{color: '#fff', fontSize : '12px',background:'rgba(0, 0, 0, 0)',borderColor:'rgba(0, 0, 0, 0)'}"
                          :offset="{width: 4,height:2 }"
                />
            </bm-marker>
            <bm-marker :position="{lng: 122.817, lat: 41.525}" :dragging="true" animation="BMAP_ANIMATION_BOUNCE" @click="lookDetail(markers[4])">
                <bm-label content="E"
                          :labelStyle="{color: '#fff', fontSize : '12px',background:'rgba(0, 0, 0, 0)',borderColor:'rgba(0, 0, 0, 0)'}"
                          :offset="{width: 4,height:2 }"
                />
            </bm-marker>
            <bm-marker :position="{lng: 123.335, lat: 41.466}" :dragging="true" animation="BMAP_ANIMATION_BOUNCE" @click="lookDetail(markers[5])">
                <bm-label content="F"
                          :labelStyle="{color: '#fff', fontSize : '12px',background:'rgba(0, 0, 0, 0)',borderColor:'rgba(0, 0, 0, 0)'}"
                          :offset="{width: 4,height:2 }"
                />
            </bm-marker>
            <bm-marker :position="{lng: 123.738, lat: 41.449}" :dragging="true" animation="BMAP_ANIMATION_BOUNCE" @click="lookDetail(markers[6])">
                <bm-label content="G"
                          :labelStyle="{color: '#fff', fontSize : '12px',background:'rgba(0, 0, 0, 0)',borderColor:'rgba(0, 0, 0, 0)'}"
                          :offset="{width: 4,height:2 }"
                />
            </bm-marker>
            <bm-marker :position="{lng: 123.555, lat: 41.886}" :dragging="true" animation="BMAP_ANIMATION_BOUNCE" @click="lookDetail(markers[7])">
                <bm-label content="H"
                          :labelStyle="{color: '#fff', fontSize : '12px',background:'rgba(0, 0, 0, 0)',borderColor:'rgba(0, 0, 0, 0)'}"
                          :offset="{width: 4,height:2 }"
                />
            </bm-marker>
            <bm-marker :position="{lng: 123.331, lat: 41.777}" :dragging="true" animation="BMAP_ANIMATION_BOUNCE" @click="lookDetail(markers[8])">
                <bm-label content="I"
                          :labelStyle="{color: '#fff', fontSize : '12px', background:'rgba(0, 0, 0, 0)',
                          borderColor:'rgba(0, 0, 0, 0)'}" :offset="{width: 4,height:2 }"
                />
            </bm-marker>
            <bm-info-window :position="{lng: a, lat: b}" title="Location" :show="infoWindow.show" @close="infoWindowClose" @open="infoWindowOpen">
                <p v-text="infoWindow.contents"></p>
            </bm-info-window>
        </baidu-map>
        <!-- 浮窗 1 -->
        <div class="graph-box-1">
            <div style="display: flex; flex-direction:column; clear: both; margin-top: 20px; height: 100px;">
                <transition name="el-zoom-in-top">
                    <div v-show="show2" class="transition-box">
                        <div id="myChart1" :style="{width: '300px', height: '300px', float: 'left'}"></div>
                    </div>
                </transition>

                <transition name="el-zoom-in-bottom">
                    <div v-show="show2" class="transition-box">
                        <div id="myChart2" :style="{width: '300px', height: '300px', float: 'left'}"></div>
                    </div>
                </transition>
            </div>
        </div>
        <!-- 时间拉环 -->
<!--        <div class="block0">-->
<!--            <el-slider-->
<!--                    vertical-->
<!--                    height="160px"-->
<!--                    show-stops-->
<!--                    :marks="marks"-->
<!--                    :max="6">-->
<!--            </el-slider>-->
<!--        </div>-->
        <div class="block" v-show="show0">
            <span class="demonstration">查询时间：  </span>
            <el-date-picker
                    v-model="value2"
                    type="datetimerange"
                    :picker-options="pickerOptions"
                    range-separator="至"
                    start-placeholder="开始日期"
                    end-placeholder="结束日期"
                    align="right">
            </el-date-picker>
            <el-button type="success" icon="el-icon-search">查询</el-button>
        </div>
        <!-- 浮窗 2 地图效果控制按键 -->
        <div class="graph-box-2">
            <div style="float: right">
                <el-button @click="show2 = !show2">可视化分析</el-button>
            </div>
            <div style="float: right">
                <el-button @click="hotMap">人口热力图</el-button>
            </div>
            <div style="float: right">
                <el-button @click="clusterCenter">聚类中心</el-button>
            </div>
            <div style="float: right">
                <el-button @click="routeManage">路线规划</el-button>
                <!-- 路线规划 -->
                <div class="graph-box-3" v-show="show3">
                    <el-form :inline="true" :model="formInline" class="demo-form-inline">
                        <el-input v-model="formInline.start" placeholder="出发地" style="width: 10em; height: 2em; margin-right: 1em" id="startAddress"></el-input>
                        <el-input v-model="formInline.end" placeholder="目的地" style="width: 10em; height: 2em; margin-right: 1em" id="endAddress"></el-input>
                        <el-form-item label="交通方式">
                            <el-select v-model="formInline.way" placeholder="出行方式" style="width: 10em">
                                <el-option label="步行" value="walk"></el-option>
                                <el-option label="自驾车" value="car"></el-option>
                                <el-option label="公交车" value="bus"></el-option>
                                <el-option label="地铁" value="railway"></el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item label="人群标签">
                            <el-select v-model="formInline.user" placeholder="我的标签" style="width: 10em">
                                <el-option label="学生" value="student"></el-option>
                                <el-option label="上班族" value="worker"></el-option>
                                <el-option label="旅行者" value="traveler"></el-option>
                                <el-option label="其他" value="others"></el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item>
                            <el-button type="primary" value='search' @click='wayRecommand'>查询</el-button>
                        </el-form-item>
                    </el-form>
                    <baidu-map class="map0" :scroll-wheel-zoom="true" center="沈阳">
<!--                        <bm-walking :start="formInline.start" :end="formInline.end"  :auto-viewport="true" location="沈阳"></bm-walking>-->
<!--                        <bm-driving-->
<!--                                :start="formInline.start" :end="formInline.end"-->
<!--                                :auto-viewport="true"></bm-driving>-->
                        <bm-transit :start="formInline.start" :end="formInline.end" :auto-viewport="true" pageCapacity="2" location="沈阳"></bm-transit>
                    </baidu-map>
                    <p></p>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
    import {BmlHeatmap} from 'vue-baidu-map'
    let myChart = {}
    var piePatternSrc = 'data:image/jpeg;base64,';
    var piePatternImg = new Image();
    piePatternImg.src = piePatternSrc;

    var itemStyle = {
        normal: {
            opacity: 0.7,
            color: {
                image: piePatternImg,
                repeat: 'repeat'
            },
            borderWidth: 3,
            borderColor: '#235894'
        }
    };
    export default {
        name: "Map",
        data () {
            return {
                //出行方式选择参数
                tableData:[

                ],
                formInline: {
                    start:'',
                    end:'',
                    user: '',
                    way: ''
                },
                //导航
                from: null,
                to: null,
                //海量点
                points: [],
                //时间拉环&日期时间段选择拉框
                // marks: {
                //     1: '00:00-4:00',
                //     2: '4:00-7:46',
                //     3: '7:46-11:05',
                //     4: '11:05-13:40',
                //     5: '13:40-16:00',
                // },
                t: '',
                pickerOptions: {
                    shortcuts: [{
                        text: '最近一周',
                        onClick(picker) {
                            const end = new Date();
                            const start = new Date();
                            start.setTime(start.getTime() - 3600 * 1000 * 24 * 7);
                            picker.$emit('pick', [start, end]);
                            console.log(start, end)
                        }
                    }, {
                        text: '最近一个月',
                        onClick(picker) {
                            const end = new Date();
                            const start = new Date();
                            start.setTime(start.getTime() - 3600 * 1000 * 24 * 30);
                            picker.$emit('pick', [start, end]);
                        }
                    }, {
                        text: '最近三个月',
                        onClick(picker) {
                            const end = new Date();
                            const start = new Date();
                            start.setTime(start.getTime() - 3600 * 1000 * 24 * 90);
                            picker.$emit('pick', [start, end]);
                        }
                    }]
                },
                value1: [new Date(2000, 10, 10, 10, 10), new Date(2000, 10, 11, 10, 10)],
                value2: '',
                //热力图的数据
                data: [
                    // ...此处添加更多的数据集
                ],
                zoom: 10,
                addressKeyword: "",
                //浮窗的数据
                show0: false,
                show1: false,
                show2: false,
                show3: false,
                show4: 0,
                //信息窗体的数据
                a:123.419112,
                b:41.808487,
                infoWindow: {
                    show: true,
                    contents: '123.41911277834251,41.808487415519224'
                },
                markers: [
                    {
                        lng: 123.419112,
                        lat: 41.808487,
                        name: 'A',
                        info:'',
                        showFlag: false //flag放在每一条数据里
                    },
                    {
                        lng: 122.91463215833338,
                        lat: 42.01317585763888,
                        name: 'B',
                        info:'',
                        showFlag: false
                    },
                    {
                        lng: 123.52380029523812,
                        lat: 42.43089657819044,
                        name: 'C',
                        info:'',
                        showFlag: false //flag放在每一条数据里
                    },
                    {
                        lng: 122.5066429,
                        lat: 41.458191109,
                        name: 'D',
                        info:'',
                        showFlag: false
                    },
                    {
                        lng: 122.81766205612237,
                        lat: 41.525756291020414,
                        name: 'E',
                        info:'',
                        showFlag: false //flag放在每一条数据里
                    },
                    {
                        lng: 123.33561641293102,
                        lat: 41.46698820258623,
                        name: 'F',
                        info:'',
                        showFlag: false
                    },
                    {
                        lng: 123.73882485,
                        lat: 41.4496345525,
                        name: 'G',
                        info:'',
                        showFlag: false //flag放在每一条数据里
                    },
                    {
                        lng: 123.55520231279614,
                        lat: 41.88656265573462,
                        name: 'H',
                        info:'',
                        showFlag: false
                    },
                    {
                        lng: 123.33181607931849,
                        lat: 41.777629081348,
                        name: 'I',
                        info:'',
                        showFlag: false //flag放在每一条数据里
                    }
                ],
            }
        },
        components: {
            BmlHeatmap
        },
        mounted(){
            //我们要在mounted生命周期函数中实例化echarts对象。因为我们要确保dom元素已经挂载到页面中
            this.drawBar();
            this.drawPie();
            // this.addPoints();
        },
        //生命周期函数
        created(){
            const that = this;
            axios.get('http://localhost:8081/people/findAll/0/10').then(function (resp) {
                console.log(resp);
                const dataset=0;
                let cs=[];
                console.log(dataset);
                that.tableData = resp.data.content;
                that.pageSize = resp.data.size;
                that.total = resp.data.totalElements;
                for(let i=0;i<7;i++){
                    //获取经度，并将字符串转换为浮点类型
                    cs.push(window.parseFloat(that.tableData[i].longitude))
                }
                that.cs = cs;
                console.log("this is that.cs",that.cs);
            })
        },
        methods: {
            //路线推荐
            wayRecommand(){
                this.$confirm('目前属于高峰时段，请错开高峰出行，建议15分钟后出行。继续出行推荐，是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$message({
                        type: 'success',
                        message: '出行推荐使用电动车，预计20分钟到达目的地'
                    });
                }).catch(() => {
                    this.$message({
                        type: 'info',
                        message: '等待15分钟'
                    });
                });
            },
            //散点分布,后期功能单人点分布
            // clickHandler (e) {
            //     alert(`单击点的坐标为：${e.point.lng}, ${e.point.lat}`);
            // },
            // addPoints () {
            //     const points = [];
            //     const that = this;
            //     // 这里必须axios获取接口，不然获取为空数组！！！
            //
            //         for (var i = 0; i < 1000; i++) {
            //             const position = {lng: Math.random() * 40 + 85, lat: Math.random() * 30 + 21};
            //             points.push(position)
            //         }
            //
            //     this.points = points
            // },
            //热力图
            hotMap(){
                this.show0 = !this.show0;
                if (this.show0==false){
                    this.data = []
                }
                else if (this.show0==true){
                    this.data = [
                        {lng: 123.419112, lat: 41.808487, count: 5551},
                        {lng: 122.91463215833338, lat: 42.01317585763888, count: 72},
                        {lng: 123.52380029523812, lat: 42.43089657819044, count: 105},
                        {lng: 122.5066429, lat: 41.458191109, count: 20},
                        {lng: 122.81766205612237, lat: 41.525756291020414, count: 98},
                        {lng: 123.33561641293102, lat: 41.46698820258623, count: 116},
                        {lng: 123.73882485, lat: 41.4496345525, count: 4},
                        {lng: 123.55520231279614, lat: 41.88656265573462, count: 211},
                        {lng: 123.33181607931849, lat: 41.777629081348, count: 1290}
                        // ...此处添加更多的数据集
                    ]
                }
            },
            clusterCenter(){
                this.show1 = !this.show1;
                if (this.show1==false){

                }
                else if (this.show1==true){

                }
            },
            routeManage(){
                this.show3 = !this.show3;
                if (this.show3==false){

                }
                else if (this.show3==true){

                }
            },
            getLocationPoint(e) {
                this.lng = e.point.lng;
                this.lat = e.point.lat;
                /* 创建地址解析器的实例 */
                let geoCoder = new BMap.Geocoder();
                /* 获取位置对应的坐标 */
                geoCoder.getPoint(this.addressKeyword, point => {
                    this.selectedLng = point.lng;
                    this.selectedLat = point.lat;
                });
                /* 利用坐标获取地址的详细信息 */
                geoCoder.getLocation(e.point, res=>{
                    console.log(res);
                });
            },
            drawBar(){
                // 基于准备好的dom，初始化echarts实例
                let myChart = this.$echarts.init(document.getElementById('myChart1'))
                // 绘制柱状图
                myChart.setOption({
                    // onresize方法的使用
                    dataset: {
                        source: [
                            ['amount', 'amount', 'time'],
                            [1727, 1727, '00:00-4:00'],
                            [1407, 1407, '4:00-7:46'],
                            [1543, 1543, '7:46-11:05'],
                            [1379, 1379, '11:05-13:40'],
                            [1419, 1419, '13:40-16:00'],
                        ]
                    },
                    grid: [{
                        width: '82%',
                        height: '50%',
                        left:'3%',
                        right:'15%',
                        containLabel: true
                    }],
                    xAxis: {
                        name: '人数',
                        fontSize: '1px',
                        axisLabel:{
                            interval:5,
                            rotate:38
                        },
                    },
                    yAxis: {
                        name: '时间',
                        type: 'category'
                    },
                    visualMap: {
                        orient: 'horizontal',
                        top: '75%',
                        left: 'center',
                        min: 0,
                        max: 2000,
                        text: ['Dense', 'Sparse'],
                        // Map the score column to color
                        dimension: 0,
                        inRange: {
                            color: ['#D7DA8B', '#E15457']
                        }
                    },
                    series: [
                        {
                            type: 'bar',
                            encode: {
                                // Map the "amount" column to X axis.
                                x: 'amount',
                                // Map the "time" column to Y axis
                                y: 'time'
                            }
                        }
                    ]
                })
            },
            drawPie(){
                // 基于准备好的dom，初始化echarts实例
                let myChart = this.$echarts.init(document.getElementById('myChart2'))
                // 绘制图表
                myChart.setOption({
                    tooltip: {},
                    series: [{
                        name: 'pie',
                        type: 'pie',
                        selectedMode: 'single',
                        selectedOffset: 30,
                        radius:'50%',
                        clockwise: true,
                        label: {
                            fontSize: 18,
                            color: '#235894'
                        },
                        labelLine: {
                            lineStyle: {
                                color: '#235894'
                            }
                        },
                        data: [
                            {value: 5551, name: 'A'},
                            {value: 72, name: 'B'},
                            {value: 105, name: 'C'},
                            {value: 20, name: 'D'},
                            {value: 98, name: 'E'},
                            {value: 116, name: 'F'},
                            {value: 4, name: 'G'},
                            {value: 211, name: 'H'},
                            {value: 1290, name: 'I'}
                        ],
                        itemStyle: itemStyle
                    }]
                });
            },
            // 点击点坐标赋值
            lookDetail(marker){
                this.a = marker.lng;
                this.b = marker.lat;
                console.log('map');
                marker.showFlag = true;
                this.infoWindow.info = marker.info;
            },
            // 关闭弹窗
            infoWindowClose(marker) {
                marker.showFlag = false
            },
            // 打开弹窗
            infoWindowOpen(marker) {
                marker.showFlag = true
            },
        }
    }
</script>

<style scoped>
    .map {
        width: 100%;
        height: 700px;
        margin-top: 20px;
    }
    /*.block0 {*/
    /*    position:fixed;*/
    /*    left:250px;*/
    /*    top:180px;*/
    /*    font-weight: bold;*/
    /*}*/
    .block {
        position:fixed;
        left:550px;
        top:50px;
    }
    .graph-box-1 {
        /*background-color: #fff;*/
        width:260px;
        height:120px;
        /*margin-top:10px;*/
        position:fixed;
        right:90px;
        top:150px;
    }
    .transition-box {
        background-color: rgba(255,255,255,0.5);
        margin: 20px 0;
        width: 300px;
        height: 270px;
        border-radius: 4px;
        text-align: center;
        color: #fff;
        box-sizing: border-box;
    }
    .graph-box-2 {
        /*background-color: #fff;*/
        width: 600px;
        height: 30px;
        margin-top: 50px;
        position: fixed;
        right: 60px;
        top: 0px;
    }
    .graph-box-3 {
        background-color: #ffffff;
        width: 85%;
        height: 670px;
        margin-top: 25px;
        position: fixed;
        left: 230px;
        top: 80px;
    }
    .map0 {
        width: 100%;
        height: 300px;
    }
    /deep/.BMap_cpyCtrl {
        display: none!important;
    }
    /deep/.anchorBL {
        display: none !important;
    }
</style>